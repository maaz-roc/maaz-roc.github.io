...
(function($) {// jscs:ignore requireNamedUnassignedFunctions
	...
	function getPropertyRowHtml(pgId, name, value, meta, postCreateInitFuncs, getValueFuncs, options, el) {
		...
		// If boolean create checkbox
			...
			const min = meta?.options?.min != null ? `min="${meta.options.min}"` : '';
			const max = meta?.options?.max != null ? `min="${meta.options.max}"` : '';
			valueHTML = `<input type="text" ${min} ${max} id="` + elemId + '" value="' + value + '" style="width:50px" />';
		...
		if (meta.colspan2) {
			return `<tr class="pgRow ${name} title="${meta.description}">` + '<td colspan="2" class="pgCell">' + valueHTML + '</td></tr>';
		} else {
			return `<tr class="pgRow ${name}" title="${meta.description}">` + '<td class="pgCell">' + displayName + '</td><td class="pgCell">' + valueHTML + '</td></tr>';
		}
	}
