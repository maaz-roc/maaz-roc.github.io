...
import * as BufferGeometryUtils from './lib/three-utils/BufferGeometryUtils.js';
import * as Earcut from "./node_modules/three/src/extras/Earcut.js";
...
type BoreholeOptions = {
    boreholeRadius : number
}
...
function getDashSize (lineThickness : number, lineStyleScale : number) {
    ...
    const dashSize = 1/4 * parseFloat(lineThickness as any) * 12 * (parseFloat(lineStyleScale as any) / 25)
    return dashSize;
}

function getLineMaterial (lineProperties : LineProperties, options = defaultOptions) {
    // we ignore the lineProperties.lineThickness since webgl doesn't support it and use the default instead
    const lineThickness = parseFloat((lineProperties.lineThickness ?? options.lineThickness) as any);
...
function getMeshLineDashSize (lineThickness : number, lineStyleScale : number) {
    lineStyleScale = Math.max(lineStyleScale, 0);
    // the equivalent scale won't look the same in 2D and 3D
    // because in 3D the section is longer than in 2D
    // but overall, lineStyleScale of 50 with line thickness of 2 in 2D will mean a dash every ~12 feet in any horizontal/vertical scale
    ...
    // apparently the real value is about 1/4 of that
    // if we want to replicate the length, we arrive at
    const dashSize = 0.005 * parseFloat(lineThickness as any) * parseFloat(lineStyleScale as any)
    return dashSize;
}

function getMeshLineMaterial (lineProperties : LineProperties, options = defaultOptions) {
    const lineThickness = parseFloat((lineProperties.lineThickness ?? options.lineThickness) as any);
...
export function getLine2DashSize (lineThickness : number, lineStyleScale : number) {
    lineStyleScale = Math.max(lineStyleScale, 0);
    const dashSize = line2DashSizeFactor * parseFloat(lineThickness as any) * parseFloat(lineStyleScale as any)
    return dashSize;
}

function getLine2Material (lineProperties : LineProperties, options = defaultOptions) {
    const lineThickness = parseFloat((lineProperties.lineThickness ?? options.lineThickness) as any);
...
function create3dEdgesLineSegments (geometry, angle=60, lineProperties : LineProperties, options=defaultOptions) {
...
function create3dEdgesMeshLine (geometry, angle=60, lineProperties : LineProperties, options=defaultOptions) {
    const edges = new THREE.EdgesGeometry(geometry, angle);
...
// returns a line object corresponding to the geometry edges
function create3dEdgesLine2 (geometry, angle=60, lineProperties : LineProperties, options=defaultOptions) {
    const edges = new THREE.EdgesGeometry(geometry, angle);
...
function create3dEdges (geometry, angle=60, lineProperties : LineProperties, options=defaultOptions) {
    const mesh = create3dEdgesLine2(geometry, angle, lineProperties, options);
    mesh.name = 'edgeLines';
    return mesh;
}

function layerTo3d (layer : Layer, options = defaultOptions, bool = true) {
    ...
    if (options.lineVisible) {
        const line = create3dEdges(layerGeometry, 60, {lineStyle : "Solid", opacity: layer.opacity} as LineProperties, options);
        layerGroup.add(line);
    }
...
export async function initializeFont(fontPath : string) {
...
export function textTo3d (text : string, options=defaultOptions, color=0x000000 as any, size = 100, useTroikaLib = false, outline = false) {
...
export function textGenerator(options=defaultOptions, color=0x000000 as any, useTroikaLib = false, outline = false) {
...
function generate3dPlane(width, height, color=0xffffff as any) {
    if (width < 0 || height < 0) {
        return null;
    }
...
function boreholeTo3d (borehole : Borehole, options = defaultOptions, size = 100, bool = true) {
...
function coordinatesToArr(str : string) : [number, number, number] {
    let [x, z, y] = str.split(',').map(x => parseFloat(x));
    let coordinates = [x, y, -z] as [number, number, number];
    return coordinates;
}
...
function crossSectionCoordinateTo3D (boreholeCoordinates1 : [number, number, number], boreholeCoordinates2 : [number, number, number], boreholeCoordinates2d1: [number, number], boreholeCoordinates2d2, crossSectionCoordinate : [number, number]) : [number, number, number] {
...
function uniqArrOfArrs(arr) {
    ...
    let arr2 = Array.from(set).map((x : string) => JSON.parse(x));
    return arr2;
}
...
function polygonTo3d(polygon : Polygon, hatchColorsDict = {}, options = defaultOptions) {
    ...
    group.add(mesh);
    if (options.lineVisible) {
        const line = create3dEdges(geometry, 60, {lineStyle : polygon.lineStyle, opacity : polygon.opacity} as LineProperties, options);
        group.add(line);
    }
...
function getPolygonCenter(points : [number, number][]) : [number, number] {
    ...
    const polygonCenter = [polygonCenterX, polygonCenterY] as [number, number];
...
function positionPolygonMesh(mesh, points, boreholesData) {
    ...
    const coordinates2d1 = [borehole1.general.x, borehole1.general.z] as [number, number];
    const coordinates2d2 = [borehole2.general.x, borehole2.general.z] as [number, number];
...
function flattenJsPolygonToPoints(polygon) : [number, number][] {
...
export function crossSectionTo3dFromData (crossSectionData, options = defaultOptions, bool = true, size = 100) {
    ...
    const boreholes = boreholesData.map(boreholeData => {
        ...
        if (properties?.fontSize) {
            borehole.titleFontSize = parseFloat(properties?.fontSize as any);
        }
    ...
    const textDataArr = Object.values(crossSectionData.texts) ?? [] as any;
...
type Wedge = {
    ...
    color?: string
    hasEdge?: boolean
    polygonOffset?: number // negative displays if overlap, positive hides
    opacity?: number
    hatchRepeat? : number
    isActive?: boolean
    renderOrder: number
...
type SupportBolt = {
    // no location if it's a surcharge
...
function supportBoltToMaterial(supportBolt : SupportBolt) {
...
function create3dCylinder(cylinderLength, radius, material, options=defaultOptions) {
    ...
    const line = create3dEdges(geometry, 60, {lineStyle : "Solid", opacity : material.opacity} as LineProperties, options);
...
function supportBoltTo3d(supportBolt : SupportBolt, modelThickness : number, lengthExtruding : number, options=defaultOptions) {
    ...
    if (options.lineVisible) {
        const line = create3dEdges(steelPlateGeometry, 60, {lineStyle : "Solid"} as LineProperties, options);
...
function angleToUnitVector2(angle : number) {
...
function angleToUnitVector3(angle : number) {
    const rad = degToRad(angle);
    const ratio = Math.tan(rad);
    const y = ratio;
    const x = 1;
...
function positionSupportBolt(group, modelThickness : number, supportBolt : SupportBolt) {
...
function getSupportBoltLength(location : [number, number], angle : number, basePolygonPoints : [number, number][]) {
...
function forceTo3dCylinder(force: SupportBolt, modelThickness:number, options=defaultOptions) {
...
function forceToArrow(force: SupportBolt, modelThickness:number, options=defaultOptions) {
    const cylinderLength = force.length ?? options.arrowLength;
...
function forceTo3d(force: SupportBolt, modelThickness:number, options=defaultOptions) {
...
function forceTo3dFromData(forceData, modelThickness:number, options=defaultOptions) {
...
function seismicTo3dFromData(forceData, modelThickness:number, options=defaultOptions) {
    const force = supportBoltDataToSupportBolt(forceData);
...
function compareEdgesByLeftPointX(e1 : [[number, number], [number, number]], e2 : [[number, number], [number, number]]) {
    // left point
    const p1 = e1[0];
    const p2 = e2[0];
...
function comparePointsByX(p1 : [number, number], p2 : [number, number]) {
    const x1 = p1[0];
    const x2 = p2[0];
    return x1 - x2;
}

function getEdgeLength(edge : [[number, number], [number, number]]) {
    const [p1, p2] = edge;
    const [x1, y1] = p1;
    const [x2, y2] = p2;
    const dx = x2 - x1;
...
function surchargeTo3d(force: SupportBolt, modelThickness:number, wedgeFace : [[number, number], [number, number]], options=defaultOptions) {
    ...
    const meshes = locations.map(location => {
        const forceClone = JSON.parse(JSON.stringify(force)) as SupportBolt;
    ...
    const unitingCylinder = JSON.parse(JSON.stringify(force)) as SupportBolt;
...
function getUpperPolygonFaces (polygonPoints : [number, number][]) {
    const uniqPoints = uniqArrOfArrs(polygonPoints);
    if (uniqPoints.length < 2) {
        return [];
    }
...
function identifyWedgeFaces(polygonPoints : [number, number][]) {
...
function surchargeTo3dFromData(forceData, wedgePolygonPoints, typeOfSurcharge : "Upper_Surcharge"|"Slope_Surcharge", modelThickness, options=defaultOptions) {
    const force = supportBoltDataToSupportBolt(forceData);
...
function wedgeTo3d(wedge : Wedge, options=defaultOptions) {
    ...
    const line = create3dEdges(geometry, 60, {lineStyle : "Solid"} as LineProperties, options);
    group.add(line);
...
function getWedgePoints(wedgeData) : [number, number][] {
    const vertices = wedgeData.VerticesArr ?? wedgeData.pointsArr as [number, number, number][];
    if (! vertices) {
        return [];
    }
...
function groupPointByIndex (points : number[][]) {
...
function getPointMaxes(points : number[][]) : number[] {
...
function getPointMins(points : number[][]) : number[] {
    const groupedByIndex = groupPointByIndex(points);
...
export function supportBoltTo3dFromRocplaneData(supportBolt : SupportBolt, rocPlaneData, options=defaultOptions) {
    const wedgesDict = rocPlaneDataToWedges(rocPlaneData, options);
    const {base, wedge, water, contactWater} = wedgesDict;
...
export function getMeshesminRealCoordinates(meshArr) : [number, number, number] {
    ...
    return [minX, minY, minZ] as [number, number, number];
}

// returns base coordinate
export function moveMeshesCloserToOrigin(meshArr) : [number, number, number] {
...
function conformFilename (filename : string) : string {
    if (! filename) {
        return '';
    }
...
function filenameToColor (filename: string) : string {
    if (! filename) {
        return '';
    }
    filename = conformFilename(filename);
...
function getMaterialDictKey(hatchColor : string, hatchSymbol : string) : string {
    return hatchColor.toLowerCase() + '_' + hatchSymbol;
}

function filenameToMaterialDictKey (filename : string) : string {
...
export function loadSvg(svgUrl : string) {
