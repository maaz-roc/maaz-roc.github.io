...
import { supportBoltTo3dFromRocplaneData, defaultOptions, wedgeSettingsToOptions, isRocPlaneData, textTo3d, textGenerator } from "./json_to_3d.js";
import { radToDeg, floatsEqual, floatsLess } from "./helperFunctions.js";
...
function addSupportBoltAtPoint(location, angle, rocPlaneData, scene, options = defaultOptions) {
    const supportBolt = {
        location: location,
...
// 3 is z
// -1 is -x
export function lookToAxis(controls, number) {
...
export function viewFromAxis(controls, number) {
    const axis = Math.abs(number);
    const direction = Math.sign(number);
    const camera = controls.object;
...
function generateGridWithTexts(xSize, zSize, divisions, realCenterCoordinates, options, size = 100, is3D = true) {
...
function generateVerticalAxis(verticalSize, verticalDivisions, sceneMinY, tickLength = 1, color = 0xffffff, options = defaultOptions, size = 100) {
...
function generateGrid(scene, minRealCoordinates, options, is3D = true, tSize = null) {
    if (!scene) {
        return null;
    }
...
export function addGrid(scene, minRealCoordinates, options, is3D = true, tSize = null) {
    if (!scene) {
        return null;
    }
...
export class Viewer3D {
    ...
    composer;
    fxaaPass;
    ssaaRenderPassPerspective;
    ssaaRenderPassOrthographic;
    outlinePass;
    rotateGridText = true;
    constructor(scene, minRealCoordinates, rootElem = document.body, options = defaultOptions) {
